//! Bench de la couche 1 FortiGate : tokenization + construction de
//! l'arbre générique sur une configuration synthétique réaliste.
//!
//! Le générateur est DÉTERMINISTE (aucun aléa) : N interfaces, M adresses,
//! P politiques, plus routes, groupes et services, dans le dialecte
//! `config`/`edit`/`set` réellement compris par l'adaptateur FortiGate.
//!
//! NOTE : le même générateur est dupliqué dans
//! `crates/calque-vendors/benches/fortigate_import.rs` (couches 1+2) —
//! les deux copies doivent rester identiques pour que les mesures soient
//! comparables.
...
fn bench_parse(c: &mut Criterion) {
    let mut g = c.benchmark_group("fortigate_parse");
    for (nom, p) in tailles() {
        let raw = config_synthetique(&p);
        let lignes = raw.lines().count() as u64;
        eprintln!(
            "[bench] configuration « {nom} » : {lignes} lignes, {} octets",
            raw.len()
        );
        g.throughput(Throughput::Elements(lignes));
        g.bench_with_input(BenchmarkId::new("parse", nom), &raw, |b, raw| {
            b.iter(|| {
                calque_parse::fortigate::parse(black_box(raw), "bench.conf")
